...
//  let rootNavIcon = require("./configFile/rootNavIcon");
...
function generateMenuFile(){
       ...
       if (fs.ensureDirSync(menu[j].name)) {
           ...
           if ( fs.ensureDirSync(firstLowerCase("pages")) ) {
              //切换目录
               changeCatalog("pages")
               //二级目录下的js文件
               generateLevel2File(menu[j].pages)
          }
       }
   }
   ...
   function generateLevel1File(pages, ScreenName) {
       ...
       for (var i = 0; i < pages.length; i++) {
           str += `import ${firstUpperCase(pages[i].name)} from "./pages/${
               pages[i].name
           }";\n`;
       }
       //页面配置
   ...
   /* 
     二级目录下的js文件
   */
   function generateLevel2File(pages){
       
       for (var i=0; i<pages.length; i++) {
           var str = "";
           fs.appendFileSync( `${pages[i].name}.js`,  originSource());
           str += `export default class ${firstUpperCase(pages[i].name)} extends React.Component {
               constructor(props){
                   super(props)
               }
               componentDidMount(){
               ...
               render(){
                   return (
                       <View>
                          <Text>${firstUpperCase(pages[i].name)}</Text>
                       </View>
                   )
               }
           }\n
               `
           fs.appendFileSync( `${pages[i].name}.js`,  str);
       }
